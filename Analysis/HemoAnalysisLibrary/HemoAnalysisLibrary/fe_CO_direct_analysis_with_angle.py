...
cwd = os.getcwd()
if "cluster" in cwd or "scratch" in cwd or "HemoSimulations" in cwd:
    prefix = ""
else:
    prefix = "test_data"
...
for ts in u.trajectory:
    ...
    for CO_segid, heme_segid in zip(CO_segids, heme_segids):
        ...
        n_atoms = {name: heme.select_atoms(f"name {name}")[0].position for name in N_names}

        normal = np.array([0, 0, 0])
        for triplet in triplets:
            AB = n_atoms[triplet[1]] - n_atoms[triplet[0]]
            AC = n_atoms[triplet[2]] - n_atoms[triplet[0]]

            # Normal vector
            normal = normal + np.cross(AB, AC)

        normal = normal/4
        theta = (np.pi/2 - np.arccos(np.dot(normal, CO_FE_vect) / (np.linalg.norm(normal) * np.linalg.norm(CO_FE_vect)))) * (180 / np.pi)
        prefix_ = f"{CO_segid}"
        frame_data[f"{prefix_}_angle"] = theta

        if dist < 6:
            frame_pocket_data[CO_segid + " Pocket"] = "A or B"
        elif 6 <= dist <= 12.5:
            if theta < 30:
                frame_pocket_data[CO_segid + " Pocket"] = "XE2"
            else:
                frame_pocket_data[CO_segid + " Pocket"] = "XE4"
        else:
...
for segid in CO_segids:
    col = f"{segid}_angle"  # change this if you want to plot something else such as distances, angles
    if col in df.columns:
        plt.plot(df['frame'], df[col], label=f'{segid}', linewidth=1)
...
plt.savefig("plane-CO-angle.png", dpi=300)
